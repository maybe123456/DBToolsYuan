...
public abstract class ZPageLoadCallback<T> extends ZCallback<T> implements SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener {
    ...
    protected void onSuccess(T data) {
        ...
        //非200即0
        if ((((JokeBean)zResponse.getShowapi_res_body()).getRet_code()) == 0) {
            if (isLoadMore) {
                    mAdapter.addData((List) ((JokeBean)zResponse.getShowapi_res_body()).getContentlist());
            } else {
                mAdapter.setNewData((List) ((JokeBean)zResponse.getShowapi_res_body()).getContentlist());
            }
